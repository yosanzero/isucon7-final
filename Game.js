...
class Game {
  ...
  async addIsu (reqIsu, reqTime) {
    try {
      ...
      const [db_isu] = await connection.query('SELECT isu FROM adding WHERE room_name = ? AND time = ?', [this.roomName, reqTime])
      
      try {
        this.updateRoomTime(connection, reqTime)
        
        if (db_isu.length === 0) {
          await connection.query('INSERT INTO adding(room_name, time, isu) VALUES (?, ?, ?)', [this.roomName, reqTime, reqIsu.toString()])
        } else {
