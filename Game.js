  ...
  async addIsu (reqIsu, reqTime) {
    try {
      ...
      try {
        ...
        const [[isu_adding]] = await connection.query('SELECT isu FROM adding WHERE room_name = ? AND time = ? FOR UPDATE', [this.roomName, reqTime])
        const now_count = typeof isu_adding === 'undefined' ? 0 : isu_adding['isu'];
        const newIsu = reqIsu.add(bigint(now_count))
        await connection.query('INSERT INTO adding(room_name, time, isu) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE isu=?', [this.roomName, reqTime, newIsu.toString(), newIsu.toString()])
